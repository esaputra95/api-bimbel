...
const getDataById = async (req: Request, res: Response) => {
    ...
    req: Request<{}, {}, {}, ClassTypeQueryInterface>,
    res: Response
    ) => {
    try {
        const query = req.query;
...
const getStudyGroup = async (req: Request, res: Response) => {
    try {
        ...
        query.name
        ? (filterStudyGroup = { ...filterStudyGroup, name: query.name })
        : null;
...
const getListStudent = async (req: Request, res: Response) => {
    try {
        ...
        const tentorId =
        res.locals.userType === "admin" || res.locals.userType === "employee"
            ? query.tentorId
            : res.locals.userId;
        ...
        for (let index = 0; index < data.length; index++) {
            ...
            for (
                let indexDetail = 0;
                indexDetail < data[index].scheduleDetails.length;
                indexDetail++
            ) {
